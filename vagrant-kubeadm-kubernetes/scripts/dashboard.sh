...
echo "Deploying the dashboard..."
  ...
  sudo -i -u vagrant helm upgrade --install kubernetes-dashboard kubernetes-dashboard/kubernetes-dashboard --create-namespace --namespace kubernetes-dashboard --version "${DASHBOARD_VERSION}"
...
sudo -i -u vagrant kubectl -n kubernetes-dashboard create token admin-user
#sudo -i -u vagrant kubectl -n kubernetes-dashboard port-forward --insecure-skip-tls-verify --address 192.168.56.10,localhost svc/kubernetes-dashboard-kong-proxy 8443:443




  sudo -i -u vagrant kubectl -n kubernetes-dashboard get secret/admin-user -o go-template="{{.data.token | base64decode}}" | sudo tee "${config_path}/token"
  echo "The following token was also saved to: configs/token"
  cat "${config_path}/token"
echo "Access Dashboard @ https://node02:8443/" 
#https://kubernetes.io/docs/tasks/access-application-cluster/web-ui-dashboard/ 

  sudo -i -u vagrant cp -r /home/vagrant/.kube /vagrant # copy over file..
fi
